...
const ViewProfileLayer = () => {
    ...
    if (error) {
        return (
            <div className="alert alert-danger shadow-sm rounded-3 mx-auto my-4" style={{ maxWidth: '600px' }} role="alert">
                    ...
                    <div className="card shadow-sm border-0 rounded-4">
                        <div className="card-body p-4">
                            <div className="text-center mb-4">
                                ...
                                <span className={`badge ${user?.is_admin ? 'bg-primary' : 'bg-secondary'} rounded-pill px-3 py-2`}>
                                    <Icon icon={user?.is_admin ? 'mdi:shield-account' : 'mdi:account'} className="me-1" width="18" />
                                    {user?.is_admin ? t('roles.administrator') : t('roles.user')}
                                </span>
                            </div>

                            <div className="profile-info bg-light rounded-4 p-4">
                                <h6 className="text-primary mb-3">{t('profile.personal_info')}</h6>
                                <div className="info-list">
                                    <InfoItem icon="mdi:email" label={t('fields.email')} value={user?.email} />
                                    <InfoItem icon="mdi:phone" label={t('fields.phone')} value={user?.phone || '-'} />
                                    <InfoItem icon="mdi:calendar" label={t('fields.joined')} 
                                        value={new Date(user?.created_at).toLocaleDateString()} />
                                    <InfoItem icon="mdi:translate" label={t('fields.language')} value={user?.language || 'English'} />
                                    <InfoItem icon="mdi:office-building" label={t('fields.department')} value={user?.department || '-'} />
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="tab-content">
                                    ...
                                    <div className="profile-edit-form">
                                        <div className="row g-3">
                                            <div className="col-md-6">
                                                <label className="form-label">{t('fields.full_name')}</label>
                                                <input 
                                                    type="text" 
                                ...
                                {activeTab === 'password' && (
                                    <div className="password-change-form">
                                        <div className="mb-3">
                                            <label className="form-label">{t('fields.new_password')}</label>
                                        ...
                                        <div className="mb-3">
                                            <label className="form-label">{t('fields.confirm_password')}</label>
                                            <div className="input-group">
                                                <input
